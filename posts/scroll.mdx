...
Scrollex let's you create beautiful scroll experiences using minimal code.
...
In our app we'll need to add a `Scroll.Container` to hold all of our scrollable content.
...
The code above dictates that before the user has scrolled at all, headings should have a translation of -200px in the x direction.

However it also dictates that after the user has scrolled down an amount of 200 pixels, then each heading should have a translation of +200 pixels in the x direction.
...
<CodeSection files={files.sectionFour}>
...
In the code above, the first and last keyframe are identical to those in the previous step, however now we've added two keyframes in between which guarantee that the heading will remain still for a range of 200 pixels as we scroll by.
...
Because `section.topAt("container-top")` returns a scroll offset value in pixels, we can add or subtract pixels from it to fine-tune our experience.

Not only can we add/subtract constants like `100`, but we could do arithmetic like the following to create a more responsive experience.

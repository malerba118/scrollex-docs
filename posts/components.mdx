...
One thing to note is that scrollex only permits us to set `transform` and `opacity` properties because they're widely known to be the [safest properties to animate](https://web.dev/animations-guide/).
